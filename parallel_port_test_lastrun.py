...
expName = 'parallel_port_test'  # from the Builder filename that created this script
expInfo = {
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/hopswork/projects/Sadism_Disgust_Code/parallel_port_test_lastrun.py',
...
p_port_test = parallel.ParallelPort(address='0x0378')
p_port = parallel.ParallelPort(address='0x0378')
text_3 = visual.TextStim(win=win, name='text_3',
    text='5 second pause between recordings',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
# update component parameters for each repeat
# keep track of which components have finished
connectComponents = [p_port_test, p_port, text_3]
...
while continueRoutine and routineTimer.getTime() < 9.0:
    ...
    # update/draw components on each frame
    # *p_port_test* updates
    if p_port_test.status == NOT_STARTED and t >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        p_port_test.frameNStart = frameN  # exact frame index
        p_port_test.tStart = t  # local t and not account for scr refresh
        p_port_test.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(p_port_test, 'tStartRefresh')  # time at next scr refresh
        # add timestamp to datafile
        thisExp.addData('p_port_test.started', t)
        p_port_test.status = STARTED
        win.callOnFlip(p_port_test.setData, int(1))
    if p_port_test.status == STARTED:
        # is it time to stop? (based on global clock, using actual start)
        if tThisFlipGlobal > p_port_test.tStartRefresh + 2.0-frameTolerance:
            # keep track of stop time/frame for later
            p_port_test.tStop = t  # not accounting for scr refresh
            p_port_test.frameNStop = frameN  # exact frame index
            # add timestamp to datafile
            thisExp.addData('p_port_test.stopped', t)
            p_port_test.status = FINISHED
            win.callOnFlip(p_port_test.setData, int(0))
    # *p_port* updates
    if p_port.status == NOT_STARTED and t >= 7.0-frameTolerance:
        # keep track of start time/frame for later
