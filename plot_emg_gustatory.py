...
def plot_gustatory_condition(
    ...
    # From data.py, "Spit Start" is 22 and "Spit Stop" is 32.
    starts = []
    stops = []
    ...
    types = []
    for tastant_block in tastants:
        for tastant_name in tastant_block:
            types.append(tastant_name)

    print(len(starts), len(stops), len(types))
    msg = "Must have same number of spit start and stop markers."
    assert len(starts) == len(stops), msg
    msg = "Must have same number of spit start markers and tastants."
