...
def plot_visual_condition(
    ...
    # From data.py, "Image Start" is label 8 and "Image Stop" is label 9.
    starts = []
    stops = []
    ...
    types = []
    for image_name in disgust_images:
        image_type = images[int(image_name.split("\\")[1].split(".")[0])]
        types.append(image_type)

    msg = "Must have same number of image start and stop markers."
    assert len(starts) == len(stops), msg
    msg = "Must have same number of image start markers and images."
    assert len(starts) == len(types), msg

    plot_name = f"{subject_id}_visual_condition"
    fig_width = 11
    fig_height = 9
    fig = plt.figure(
