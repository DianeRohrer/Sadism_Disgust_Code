...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/themorrigan/Desktop/Sadism_Disgust_Code/rcs_version/Gustatory_Condition_lastrun.py',
...
# --- Initialize components for Routine "Palatability_Score" ---
Palatability_Scale = visual.Slider(win=win, name='Palatability_Scale',
...
Instruction = visual.TextStim(win=win, name='Instruction',
    text='Please rate how good or bad the liquid you recevied was. ',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
for thisBlocks_of_Tastant in Blocks_of_Tastants:
    ...
    for thisEach_Tastant_Once in Each_Tastant_Once:
        ...
        routineForceEnded = False
        # update component parameters for each repeat
        # Run 'Begin Routine' code from get_spit_EMG
        ...
        # --- Prepare to start Routine "Palatability_Score" ---
        continueRoutine = True
        routineForceEnded = False
        # update component parameters for each repeat
        Palatability_Scale.reset()
        # keep track of which components have finished
        Palatability_ScoreComponents = [Palatability_Scale, Instruction]
        for thisComponent in Palatability_ScoreComponents:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "Palatability_Score" ---
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # *Palatability_Scale* updates
            if Palatability_Scale.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                Palatability_Scale.frameNStart = frameN  # exact frame index
                Palatability_Scale.tStart = t  # local t and not account for scr refresh
                Palatability_Scale.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Palatability_Scale, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'Palatability_Scale.started')
                Palatability_Scale.setAutoDraw(True)
            
            # Check Palatability_Scale for response to end routine
            if Palatability_Scale.getRating() is not None and Palatability_Scale.status == STARTED:
                continueRoutine = False
        ...
        # --- Ending Routine "Palatability_Score" ---
        for thisComponent in Palatability_ScoreComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        Each_Tastant_Once.addData('Palatability_Scale.response', Palatability_Scale.getRating())
        Each_Tastant_Once.addData('Palatability_Scale.rt', Palatability_Scale.getRT())
        # the Routine "Palatability_Score" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
