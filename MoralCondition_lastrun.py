...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/themorrigan/Desktop/Sadism_Disgust_Code/MoralCondition_lastrun.py',
...
player_image_intro = visual.ImageStim(
    ...
    ori=0.0, pos=(-.75, .5), size=(None, None),
...
player_name_offer = visual.TextBox2(
     win, text=opponent_names.pop(0), font='Open Sans',
     pos=(.75, .5),     letterHeight=0.08,
     size=(None, None), borderWidth=2.0,
     color='white', colorSpace='rgb',
     opacity=None,
...
for thisLoop in loop:
    ...
    # Run 'Begin Routine' code from get_acceptance_ENG
    RECORDING_STARTED = start_EMG_recording("Acceptance")
    # keep track of which components have finished
    ...
    for thisComponent in OfferComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    # Run 'End Routine' code from get_acceptance_ENG
    stop_EMG_recording(RECORDING_STARTED)
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
